...
/**
 * Research Queueが空になるまで待機し、
 * 未送信データがあれば送信する。
 *
 * 外部ページへ遷移する直前など、
 * 研究データの送信完了を保証したい場合に使用する。
 *
 * @returns {Promise<Object>}
 */
async function flushResearchRecords() {
    ...
    if (
      ...
      return {
        ...
        remainingCount:
          researchStorage.getQueueCount(),
        message:
          "研究データの送信完了待機がタイムアウトしました。",
      };
    }

    if (!dispatchStatus.isDispatching) {
      const result =
        await researchDispatcher.sendAll();
